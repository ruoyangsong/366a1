...
#the list to store q*(a) where a = 1,...10
...
def env_step(this_action): # returns (floating point, NumPy array, Boolean), this_action: NumPy array
    ...
    for i in range(10):
        real_reward[i] = rand_norm(q[i], 1.0)
    the_reward = real_reward[int(this_action)]
...
def env_message(inMessage): # returns string, inMessage: string
    if inMessage == "what is your name?":
        ...
        return max(enumerate(real_reward),key=lambda x: x[1])[0]
